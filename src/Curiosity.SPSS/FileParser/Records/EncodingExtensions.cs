...
{
    /// <summary>
    ///     Extensions for encoding methods to help with string writing/reading in spss formats
    /// </summary>
    internal static class EncodingExtensions
    {
        ...
        {
            if (enc == null) throw new ArgumentNullException(nameof(enc), "No encoding set");
        ...
        /// <remarks>
        ///     If the last char to be encoded does not fit entirely on the array, it will bi removed and the space remaining
        ///     will be filled with the padding byte.
        /// </remarks>
        /// <returns>The fixed length byte[]</returns>
        internal static byte[] GetPadded(this Encoding enc, string value, int length, byte padding = 0x20)
        {
            ...
            var charArr = AsCharArr(enc, value, length, out var lastCharIndex);
        ...
        ///     The max length of the resulting byte[], should be a multiple of <see cref="roundUpBytes" />
        /// </param>
        /// <param name="padding">The padding character to use (if there are remining bytes on the array)</param>
        /// <param name="length">
        ///     Used to report back the actual length of the encoded string in bytes, with out the added roundUp
        ///     bytes
        /// </param>
        /// <param name="roundUpDelta">
        ///     For when the round up must be performed for the string and some more characters.
        ///     When specified, the original encoded byte length + this  will be the multiple of <see cref="roundUpBytes" />
        /// </param>
        /// <remarks>
        ///     If the last char to be encoded does not fit entirely on the array, it will bi removed and the space remaining
        ///     will be filled with the padding byte.
        ...
        internal static byte[] GetPaddedRounded(this Encoding enc, string value, int roundUpBytes, out int length,
            int maxLength = int.MaxValue, byte padding = 0x20, int roundUpDelta = 0)
        {
            ...
            if (maxLength != int.MaxValue && maxLength % roundUpBytes != 0) throw new ArgumentException("The max length should be a multiple of the round up bytes", nameof(maxLength));

            var charArr = AsCharArr(enc, value, maxLength, out var lastCharIndex, out var byteCount);
            length = byteCount;

            // Get the new round up length
        ...
        internal static int GetUpToMaxLength(this Encoding enc, string value, int maxLength, byte[] byteArr, int byteIndex)
        {
            var charArr = AsCharArr(enc, value, maxLength, out var lastCharIndex);
        ...
        /// <summary>
        ///     Gets the string as an array of chars. The resulting array has already been sliced
        ///     to the amount of chars that will fit into a byte[] of size <see cref="length" /> when
        ///     encoded with <see cref="enc" />
        /// </summary>
        /// <param name="enc">To test the length of the encoded characters</param>
        /// <param name="value">The string to be encoded</param>
        /// <param name="length">
        ///     The max length of bytes, by using <see cref="enc" /> <see cref="Encoding.GetByteCount(char[])" />
        ///     whe can check how many of the resulting char[] would fit on an encoded byte array of this length
        /// </param>
        /// <param name="lastCharIndex">
        ///     The index of up to which character should be encoded to produce a byte[] of <see cref="length" />
        /// </param>
        /// <returns>
        ///     All the chars of value. You should, use the <see cref="lastCharIndex" /> to indicate up to which
        ...
        {
            return AsCharArr(enc, value, length, out lastCharIndex, out _);
        }
        ...
        private static char[] AsCharArr(Encoding enc, string value, int length, out int lastCharIndex, out int byteCount)
        {
            ...
            // Up to which char array index should be converted (to fifth the length)
        ...
        /// <summary>
        ///     Gets the encoded representation of the char array.
        /// </summary>
        ...
        private static byte[] GetPaddedByteArray(Encoding enc, int length, byte padding, char[] charArr, int lastCharIndex)
        {
            ...
            // Get the bytes
            var writtenLength = enc.GetBytes(charArr, 0, lastCharIndex, byteArr, 0);

            for (var i = writtenLength; i < byteArr.Length; i++) byteArr[i] = padding;
            return byteArr;
        }
    }
}