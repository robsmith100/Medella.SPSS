...
namespace Curiosity.SPSS.FileParser.Records
{
    public abstract class VariableDataInfoRecord<T> : BaseInfoRecord
    {
        ...
        protected VariableDataInfoRecord(IDictionary<string, T?> dictionary, Encoding encoding)
        {
            Encoding = encoding;
            ItemSize = 1;
        ...
        public IDictionary<string, T?> Dictionary
        {
            get
            {
                if (_dictionary == null)
                {
                    if (Data == null || Data.Length == 0) throw new SpssFileFormatException("Info record dictionary has no data");
                    ...
                    do
                    {
                        ...
                        if (separatorIndex == -1) throw new SpssFileFormatException("Info record dictionary has no '=' char");
                        var endIndex = Array.IndexOf(Data, TabChar, separatorIndex);
                        var shortName = Encoding.GetString(Data, startIndex, separatorIndex - startIndex);
                        var stringValue = Encoding.GetString(Data, separatorIndex + 1, (endIndex != -1 ? endIndex : Data.Length) - separatorIndex - 1);
        ...
        protected void BuildDataArray()
        {
            ...
            foreach (var variable in _dictionary)
            {
                ...
                byteIndex += Encoding.GetUpToMaxLength(EncodeValue(variable!.Value!), 64, buffer, byteIndex);
                buffer[byteIndex++] = TabChar;
            }

            if (!UsesTerminator)
                // Do not account for the trailing tab
                byteIndex--;
