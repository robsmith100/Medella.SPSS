...
namespace Curiosity.SPSS.FileParser.Records
{
    internal static class ReaderWriterExtensions
    {
        ...
        {
            var recordTypeNum = reader.ReadInt32();
            if (!Enum.IsDefined(typeof(RecordType), recordTypeNum)) throw new SpssFileFormatException("Record type not recognized: " + recordTypeNum);
