...
namespace Curiosity.SPSS.FileParser.Records
{
    ...
    public class ValueLabelRecord : EncodeEnabledRecord, IRecord
    {
        ...
        public void WriteRecord(BinaryWriter writer)
        {
            ...
            foreach (var (value, (length, label)) in _labelsRaw!)
            {
            ...
            foreach (var dictionaryIndex in Variables) writer.Write(dictionaryIndex);
        }
        ...
        public void FillRecord(BinaryReader reader)
        {
            ...
            for (var i = 0; i < LabelCount; i++)
            {
                ...
                // labelLength + labelByte must be multiples of 8
                var labelBytes = Common.RoundUp(labelLength + 1, 8) - 1;
                var chars = reader.ReadBytes(labelBytes);

                _labelsRaw.Add(value, new KeyValuePair<byte, byte[]>((byte) labelLength, chars));
            }

            // Parse the adjacent ValueLabelVariablesRecord as well:
            // TODO improve conversion to enum, as in SavFileParser
            var type = reader.ReadRecordType();

            if (type != RecordType.ValueLabelVariablesRecord) throw new SpssFileFormatException();

            VarCount = reader.ReadInt32();
            Variables = new Collection<int>();

            for (var i = 0; i < VarCount; i++) Variables.Add(reader.ReadInt32());
        }
        ...
        private string DecodeLabel(KeyValuePair<byte, byte[]> p) => Encoding.GetString(p.Value, 0, p.Key);

        private KeyValuePair<byte, byte[]> EncodeLabel(string value)
        {
            var bytes = Encoding.GetPaddedRounded(value, 8, out var length, 120, roundUpDelta: 1);
