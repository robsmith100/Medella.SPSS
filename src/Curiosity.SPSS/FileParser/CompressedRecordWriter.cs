...
namespace Curiosity.SPSS.FileParser
{
    ...
    public class CompressedRecordWriter : IRecordWriter
    {
        ...
        ///     When this reaches a multiple of 8, a new <see cref="UncompressedValue" /> has to be written
        ///     to the compressed block, and check if the uncompressed buffer has to be flushed.
        /// </summary>
        private int _uncompressedIndex;

        /// <summary>
        ///     Creates a compressed record writer
        /// </summary>
        ...
        public void WriteNumber(double d)
        {
            ...
            if (!WriteCompressedValue(d))
                // If it couldn't be compressed, write into the uncompressed buffer its bytes
        ...
        /// <param name="length">Amount of bytes to write from the buffer</param>
        ...
        {
            // Close the uncompressed block (if there where any remaining bytes on it)
            // and add the written bytes
            writtenBytes += CloseUncompressedBufferBlock();
            ...
            if (writtenBytes != length)
                throw new Exception("Wrong count of bytes written for string variable. Expecting to write " + length +
                                    " but end up writing " + writtenBytes + " bytes");
        }
        ...
        /// <remarks>
        ///     In case this method has returned false, a <see cref="UncompressedValue" /> flag was written into the compression
        ///     block,
        ...
        {
            if (bytes.Length != 8) throw new ArgumentException("Uncompressed numbers must have an 8 bytes representation", nameof(bytes));
        ...
        /// <summary>
        ///     Gets when there is an uncompleted block on the uncompressed buffer
        /// </summary>
        /// <returns>True if there is an uncompleted block on the uncompressed buffer, false otherwise</returns>
        private bool AreUncompletedUncompressedBlocks() => _uncompressedIndex % Constants.BlockByteSize != 0;
        ...
        {
            if (AreUncompletedUncompressedBlocks()) throw new Exception("Blocks on the uncompressed buffer must be all filled before starting to write another value");
        }
        ...
        private static int FullBlocksCount(int uncompressedBytes) => uncompressedBytes / Constants.BlockByteSize;

        /// <summary>
        ///     Fills up the last uncompressed block (if there is an uncompleted block)
        /// </summary>
        /// <returns>
        ///     The count of chars that have to be written to complete the block (0 if no uncompleted block
        ///     is found, otherwise a max of 7)
        /// </returns>
        private int CloseUncompressedBufferBlock()
        {
            // If there are no uncompleted uncompressed blocks, do nothing
            if (!AreUncompletedUncompressedBlocks()) return 0;

            // Get the end of the uncompleted uncompressed buffer block
            var blockBoundary = Common.RoundUp(_uncompressedIndex, Constants.BlockByteSize);
            // Fill the remaining bytes with padding spaces
            for (var i = _uncompressedIndex; i < blockBoundary; i++) _uncompressedBuffer[i] = 0x20;
            var bytesWritten = blockBoundary - _uncompressedIndex;
        ...
        /// <summary>
        ///     Check if the compressed block is full and flush the uncompressed buffer to the writer if so.
        ///     This method also move any uncompleted block data to the beginning and set <see cref="_uncompressedIndex" />
        ///     correspondingly
        /// </summary>
        private void CheckBlock()
        {
            switch (_blockIndex)
            {
                ...
                case < Constants.BlockByteSize:
                    return;
                // We should have written up to 8 compressed code blocks before flushing, if not, there's something wrong
                case > Constants.BlockByteSize:
                    throw new Exception("A compressed block size must be no longer than 8. Current: " + _blockIndex);
            }

            // Reset compressed block index
            _blockIndex = 0;
            ...
            // Reset uncompressed buffer index, or set to the remaining bytes
        ...
        private void CompleteBlock()
        {
            ...
            // if there's at least one compressed code on the last block, fill it with padding and check for flush
            for (var i = _blockIndex; i < Constants.BlockByteSize; i++) WriteCompressedCode(Padding);
            CheckBlock();
        }
    }
}