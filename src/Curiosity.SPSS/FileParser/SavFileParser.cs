...
namespace Curiosity.SPSS.FileParser
{
    public class SavFileParser : IDisposable
    {
        ...
        public IEnumerable<byte[][]> DataRecords
        {
            get
            {
                if (!MetaDataParsed) ParseMetaData();
                lock (Stream!)
                {
                    // dataStartPosition == 0 -> Stream.Position not supported
        ...
        public void ParseMetaData()
        {
            ...
            try
            {
                ...
                if (readRecordType != RecordType.HeaderRecord) throw new SpssFileFormatException("No header record is present. A header record is required. Is this a valid SPSS file?");
        ...
        {
            var record = new byte[MetaData!.VariableRecords.Count][];
            for (var i = 0; i < MetaData.VariableRecords.Count; i++)
            {
                // TODO check unexpected eof?
                record[i] = _reader!.ReadBytes(Constants.BlockByteSize);
                if (record[i].Length < Constants.BlockByteSize) return null;
            }
        ...
        {
            // Decoder for strings
            var dec = MetaData!.DataEncoding!.GetDecoder();
            ...
            for (var variableIndex = 0; variableIndex < variableCount;)
            {
                // Variable record that correspond to the current 8 bytes block
                // Current 8 bytes block
                var element = MoveNext(record, variableRecords, out var variableRecord, ref variableIndex);
                ...
                else if (variableRecord.Type > 0)
                {
                    ...
                    if (veryLongStrings.ContainsKey(variableRecord.Name!)) segments = VariableRecord.GetLongStringSegmentsCount(veryLongStrings[variableRecord.Name!]);
                    ...
                    do
                    {
                        ...
                        while (bytesRead < length && variableRecords[variableIndex].Type == -1 && variableIndex < variableCount)
                        {
                            ...
                            var lengthRead = element.Length;
                            if (++inSegmentIndex == 32)
                            {
                                // Subtract one from the read length to ignore the last bye
                                lengthRead--;
                        ...
                        // take the segment's string we were building (the buffer up to the written index)
        ...
        protected virtual void Dispose(bool disposing)
        {
            ...
            if (_dataRecordStream == null) return;
        ...
        public Collection<Variable> Variables
        {
            get
            {
                if (MetaData == null)
                    ParseMetaData();
                if (_variables == null) GetVariablesFromRecords();
                return _variables!;
            }
        }
        ...
        /// <summary>
        ///     Creates a <see cref="Variable" /> object with it's actual information
        /// </summary>
        /// <param name="variableIndex">The actual index of the variable</param>
        /// <param name="dictionaryIndex">The index of the variable's <see cref="VariableRecord" /></param>
        /// <param name="metaData">The parsed metadata with all needed info from the file</param>
        /// <param name="length">The string length in bytes (only needed for string vars)</param>
        ...
        private static Variable GetVariable(int variableIndex, int dictionaryIndex, MetaData metaData, int length, int segmentIndex)
        {
            ...
            for (var i = 0; i < variableRecord.MissingValues!.Count && i < variable.MissingValues.Length; i++) variable.MissingValues[i] = variableRecord.MissingValues[i];


            if (variable.Type == DataType.Text)
            {
                variable.TextWidth = metaData.VeryLongStringsDictionary.TryGetValue(variableRecord.Name!, out var longLength) 
                    ? longLength 
                    : length;
            }

            // TODO: There can be one value label for multiple variables, we might want to only create one and reference it from all variables
            // Get value labels:
            ...
            if (valueLabelRecord != null)
                foreach (var (keyBytes, strValue) in valueLabelRecord.Labels)
                {
                    var key = BitConverter.ToDouble(keyBytes, 0);
                    var value = strValue.Replace("\0", string.Empty).Trim();
        ...
        private static string GetLongName(MetaData metaData, VariableRecord variableRecord)
        {
            ...
            if (metaData.LongVariableNames != null
                && metaData.LongVariableNames.Dictionary.TryGetValue(variableRecord.Name!, out var longName))
                return longName!;
        ...
        private void GetVariablesFromRecords()
        {
            ...
            // Amount of variables to jump (to skip variable continuation records and additional very long string segments)
            int delta;
            ...
            for (var dictionaryIndex = 0; dictionaryIndex < MetaData.VariableRecords.Count; dictionaryIndex += delta)
            {
                var record = MetaData.VariableRecords[dictionaryIndex];
                // If it's a string continuation record (SCR). This ones should have been skipped.
                if (record.Type < 0) throw new SpssFileFormatException("String continuation record out of place. Dictionary index " + dictionaryIndex);

                // Actual byte length for variable
                int length;
                // Amount of segments (for VeryLongString variables, 1 for all other vars).
                int segments;
                if (veryLongStrings.ContainsKey(record.Name!))
                {
                    ...
                    // Calculate the amount of segments and the amount of VariableRecords to skip (SCR)
                ...
                else
                {
                    ...
                    // numeric type is 0 so length is 1, > 0 for the length of strings
