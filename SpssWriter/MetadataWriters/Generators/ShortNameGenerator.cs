...
public class ShortNameGenerator
{
    ...
    private void GenerateVariableGhostNames(List<VariableWrapper> records)
    {
        foreach (var record in records)
        {
            ...
            {
                var sn = _encoding.GetString(GetShortNameByteArray(record.Name)).TrimEnd();
    ...
    {
        return name == null ? new byte[8] : _encoding.GetPaddedValueAsByteArray(name.ToUpperInvariant(), 8);
    }
}