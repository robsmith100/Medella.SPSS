...
namespace Curiosity.SPSS.Tests
{
    public class TestSpssWriter
    {
        ...
        public void TestWriteString()
        {
            ...
            {
                using var writer = new SpssWriter(fileStream, variables, options);
                // If the data record is not ready available, you can use: object[] newRecord = writer.CreateRecord();
                foreach (var row in data) writer.WriteRecord(row);
...
#pragma warning restore 618
        ...
        {
            const string filename = @"testWriteDoubleLongString.sav";
            ...
            var readFileStream = new FileStream(filename, FileMode.Open, FileAccess.Read, FileShare.Read, 2048 * 10, FileOptions.SequentialScan);

            TestSpssReader.ReadData(readFileStream, out var varCount, out var rowCount);
